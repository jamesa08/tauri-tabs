 ...
 */
export async function dragAsWindow(el: string | HTMLElement, onDrop?: (result: CallbackPayload) => void): Promise<void> {
    const element = typeof el === "string" ? document.querySelector(el) : el;
    if (element === null) {
        throw new Error(`Element with selector "${el}" not found`);
    }
    // add hover class before taking the screenshot
    element.classList.add("dragging");

    const canvas = await html2canvas(element as HTMLElement, { logging: false, scale: 1 });
    
    // remove hover class after taking the screenshot
 ...
 */
export async function dragBack(el: string | HTMLElement, data: any, onEvent?: (result: CallbackPayload) => void): Promise<void> {
    const element = typeof el === "string" ? document.querySelector(el) : el;
    if (element === null) {
        throw new Error(`Element with selector "${el}" not found`);
    }
